...
// $Id: mozilekb.js 1298 2005-05-04 12:03:40Z chregu $
...
function nonctrlKeyPressHandler(event)
{
	...
	if(event.charCode)
	{
		...
		var bxid = par.getAttribute("__bxe_id");
		if (!bxid) {
			return false;
		}
...
//				sel.anchorNode.parentNode.updateXMLNode();
		...
		if (par.XMLNode._node.nodeType == 2 ) {
			
			par.XMLNode._node.value =  par.getContent();
		} else {
...
function bxe_deleteEventKey(sel, backspace) {
		...
		if ((!sel.isCollapsed && sel.focusNode.nodeType == 3 && sel.focusNode.compareDocumentPosition(sel.anchorNode) & 4) ||
